...
func TestCreateRepoErrorRestclient(t *testing.T) {
	...
	response, err := CreateRepo("", github_domain.CreateRepoRequest{})
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			...
			Body: ioutil.NopCloser(strings.NewReader(`{
				...
				"full_name": "accexs/Golang-development"
				}`)),
		},
	})
	...
	assert.EqualValues(t, "Golang-development", response.Name)
	assert.EqualValues(t, "accexs/Golang-development", response.FullName)
}
